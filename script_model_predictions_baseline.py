...
class COVID_Predictor_baseline():
    """"""
    def __init__(self):
    ...
    def Predict(self, start_date, end_date, training_set, test_set, NB_LOOKBACK_DAYS=20, verbose=False):
        ...
        hist_variables_df = training_set[self.ID_COLUMNS + self.IP_COLUMNS]
        #Get variables from testing set period

        test_set = test_set[(test_set.Date >= start_date) & (test_set.Date <= end_date)]
        variables_df = test_set[self.ID_COLUMNS + self.IP_COLUMNS]
        ...
        for g in variables_df.GeoID.unique():
            ...
            past_vars= np.array(hist_variables_df[self.IP_COLUMNS])
            future_vars = np.array(vars_gdf[self.IP_COLUMNS])
